    ...
    stage('Test coverage') { // Perform unit testing
      steps {
        script {
          ...
          docker run -i -v /shared:/shared --env-file .env-test  --network recipesmountain_jenkinsci_default --link  postgres-recipemountain:database backend-test '/venv/bin/coverage' 'html' '-d' '/shared/\$filehash' 
          """          // coverage html
        }
